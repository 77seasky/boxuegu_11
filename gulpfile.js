...
// 把CommonJS规范编写的模块进行打包，打包后的文件可运行在浏览器环境。
var browserify = require('browserify');
// 这个包可以把普通的数据流转为vinyl对象文件格式
...
/**
 * 打包CommonJS模块
 * 1.其中src/js/common目录下的文件不需要打包，因为将来那个页面脚本需要他，require它即可，
 * 只要require了，就自动打包到对应的页面脚本。
...
gulp.task('js', function() {
    jsModules.forEach(function(jsPath) {
        pathArr = jsPath.split('/'); //bian成['src','js','user','login.js]
        jsName = pathArr.pop(); // 取出login.js，数组变成['src','js','user'] 
